...
const ChangeItemWrap = ({ children, className, onClick }: { children: React.ReactNode, className?: string, onClick?: () => void }) => {
  return (
    <div className={cn(
      "border border-[#232427] bg-[#1A1B1E] h-[24px] flex items-center justify-center text-[12px] text-[#9DA3AF] font-normal hover:bg-[#383A40] hover:border-[#383A40] hover:text-white cursor-pointer px-2",
      className
    )}
...
const ChangeItem = ({ change, sy, className, onClick }: { change: any, sy: string, className?: string, onClick?: () => void }) => { 
  return (
    <ChangeItemWrap className={cn("w-[61px] flex items-center", className)} onClick={onClick}>
...
const PriceChangeTab = ({
  ...
  return (
    <div className={cn(
      ...
      >
        <LazyImage src="/images/v2/icons/swap.png" className="w-[14px] h-[14px]" alt="swap" />
      </ChangeItemWrap>
    </div>
  )
}
